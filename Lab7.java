...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private void triangularPrism(GL2 gl2) {
		...
		gl2.glTexCoord2d(0,0.50);
		gl2.glVertex3d(t,0.5,-0.25);
		gl2.glTexCoord2d(0.50,1);
		gl2.glVertex3d(0,0.5,0.5);
		gl2.glEnd();
		
		gl2.glBegin(GL2.GL_TRIANGLES);  // bottom triangular
		gl2.glNormal3d(0,-1,0);
		gl2.glTexCoord2d(0.33,1);
		gl2.glVertex3d(t,-0.5,-0.25);
		gl2.glTexCoord2d(0.33,0.50);
		gl2.glVertex3d(-t,-0.5,-0.25);
		gl2.glTexCoord2d(0.85,1);
		gl2.glVertex3d(0,-0.5,0.5);
		gl2.glEnd();
		...
		gl2.glVertex3d(-t,0.5,-0.25);
		gl2.glTexCoord2d(0.67,1);
		gl2.glVertex3d(t,0.5,-0.25);
		gl2.glTexCoord2d(0.67,0);
		gl2.glVertex3d(t,-0.5,-0.25);
		gl2.glEnd();
	...
	private Texture textureFromPainting() {
		// TODO: write this method
		//will restore the image that the painPanel.copyOSC() will return 
		...
		GL2 gl2 = context.getGL().getGL2();
		      ...
		      // code that uses gl2!
		Texture texture;
		ImageUtil.flipImageVertically( paintPanel.copyOSC() );
		texture = AWTTextureIO.newTexture(displayGL.getGLProfile(),paintPanel.copyOSC(), true);
		texture.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_S, GL2.GL_REPEAT);
		texture.setTexParameteri(gl2, GL2.GL_TEXTURE_WRAP_T, GL2.GL_REPEAT);
	...
	public void display(GLAutoDrawable drawable) {    
		...
		if (currentTexture == null) { // will check if the currentTexture is null 
			gl2.glDisable( GL2.GL_TEXTURE_2D ); // will not add any texture to the figures 
		} else {
			gl2.glEnable( GL2.GL_TEXTURE_2D ); // will allow an object to display a type of texture among the options
			gl2.glMatrixMode(GL2.GL_TEXTURE);
			gl2.glLoadIdentity();
			if (textureRepeatHorizontal == 1) {
				gl2.glScaled(1, 1, 1);
			} else if (textureRepeatHorizontal == 2) {
				gl2.glScaled(2, 2, 2);
			} else if (textureRepeatHorizontal == 3) {
				gl2.glScaled(3, 3, 3);
			} else if (textureRepeatHorizontal == 4) {
				gl2.glScaled(4, 4, 4);
			}
			
			if (textureRepeatVertical == 1) {
				gl2.glScaled(1, 1, 1);
			} else if (textureRepeatVertical == 2) {
				gl2.glScaled(2, 2, 2);
			} else if (textureRepeatVertical == 3) {
				gl2.glScaled(3, 3, 3);
			} else if (textureRepeatVertical == 4) {
				gl2.glScaled(4, 4, 4);
			}
			gl2.glMatrixMode(GL2.GL_MODELVIEW);
			
			currentTexture.bind(gl2); // will allow for texture to be apply in the object in display 	
		}
